...
function ensureEditNotice(dateInput, kind) {
  const id = `${kind}EditNotice`;
  let notice = document.getElementById(id);
  if (!notice) {
    notice = document.createElement("div");
    notice.id = id;
    notice.className = "edit-notice";
    dateInput.parentElement.insertAdjacentElement("afterend", notice);
  }
  const date = dateInput.value || today();
  const exists = kind === "food" ? Boolean(foodRecords()[date]) : Boolean(bpRecords()[date]);
  notice.classList.toggle("editing", exists);
  notice.textContent = exists
    ? `${jpDate(date)} の記録を編集中です。保存すると同じ日の記録を上書きします。`
    : `${jpDate(date)} の新しい記録を入力しています。`;
}

function confirmDailySave(kind, date, exists) {
  const label = kind === "food" ? "食事・評価" : "血圧";
  const action = exists ? "上書き保存" : "新規保存";
  return confirm(`${jpDate(date)} の${label}データを${action}します。\n対象日と入力内容をご確認ください。`);
}

function editDailyRecord(kind, date) {
  if (kind === "food") {
    foodDate.value = date;
    fillFoodInputs(date);
    ensureEditNotice(foodDate, "food");
    show("foodInput");
  } else {
    bpDate.value = date;
    fillBpInputs(date);
    ensureEditNotice(bpDate, "bp");
    show("bpInput");
  }
...
saveFood.onclick = async () => {
  ...
  const exists = Boolean(records[date]);
  if (!confirmDailySave("food", date, exists)) return;
  records[date] = {
    energy: num("energy"),
    protein: num("protein"),
    salt: num("salt"),
  ...
  alert(`${jpDate(date)} の食事・評価データを${exists ? "上書き" : "保存"}しました。`);
  show("wifeView");
};
...
loadFood.onclick = () => {
  fillFoodInputs(foodDate.value || today());
  ensureEditNotice(foodDate, "food");
};
foodDate.onchange = () => {
  fillFoodInputs(foodDate.value || today());
...
saveBp.onclick = async () => {
  ...
  const exists = Boolean(records[date]);
  if (!confirmDailySave("bp", date, exists)) return;
  records[date] = {
  ...
  await pushToCloud({ silent: true });
  alert(`${jpDate(date)} の血圧データを${exists ? "上書き" : "保存"}しました。`);
  renderHistory();
};
...
loadBp.onclick = () => {
  fillBpInputs(bpDate.value || today());
  ensureEditNotice(bpDate, "bp");
};
bpDate.onchange = () => {
  fillBpInputs(bpDate.value || today());
...
function renderHistory() {
  ...
  foodHistory.innerHTML = foodKeys.length
    ? foodKeys.map((date) => {
      ...
      return `<div class="history-item">
        ...
        <span class="history-actions"><b>${score}点</b><button type="button" class="history-edit" data-edit-kind="food" data-edit-date="${date}">編集</button></span>
      </div>`;
    }).join("")
  ...
  bpHistory.innerHTML = bpKeys.length
    ? bpKeys.map((date) => `<div class="history-item">
        ...
        <button type="button" class="history-edit" data-edit-kind="bp" data-edit-date="${date}">編集</button>
      </div>`).join("")
    : "<p class='hint'>血圧履歴はまだありません。</p>";

  renderAverages();
  document.querySelectorAll("[data-edit-kind][data-edit-date]").forEach((button) => {
    button.addEventListener("click", () => editDailyRecord(button.dataset.editKind, button.dataset.editDate));
  });
}
